import { Sex, fakerRU as faker } from "@faker-js/faker";
import log4js from "log4js";
// @ts-nocheck
import { v4 as uuid } from "uuid";
import type { HelpRequestRepository } from "../HelpRequest";
...
export class UserRepository {
	...
	constructor(private readonly requestRepository: HelpRequestRepository) {
	...
	public addRequestToFavourites(requestId: string, userId: string): void {
		...
		if (!this.users[userId]) {
			logger.error(
				"UserRepository.addRequestToFavourites. Error. No user found",
				userId,
			);
		...
		if (!this.requestRepository.checkIsRequestExist(requestId)) {
			logger.error(
				"UserRepository.addRequestToFavourites. Error. No request found",
				userId,
				requestId,
			);
	...
	public removeRequestFromFavourites(requestId: string, userId: string): void {
		if (
			this.users[userId]?.favouriteRequests?.length &&
			this.users[userId].favouriteRequests.includes(requestId)
		) {
			this.users[userId].favouriteRequests = this.users[
				userId
			].favouriteRequests.filter((id) => id !== requestId);
		}
	}
